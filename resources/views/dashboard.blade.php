...
<div class="flex flex-col gap-8 text-slate-800 pb-12">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-700">Dasbor Global</h1>
            <p class="text-base text-slate-400 font-semibold mt-1">Ringkasan risiko rantai pasok global</p>
        </div>
 
        <div class="flex items-center gap-4 w-full md:w-auto">
            <!-- Search Bar -->
            <div class="relative w-full md:w-80">
                <input type="text" placeholder="Cari negara, pelabuhan, berita..." class="w-full bg-white/60 border border-white/80 backdrop-blur-md rounded-2xl py-2.5 pl-10 pr-4 text-base font-semibold text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-[0_4px_20px_rgba(0,0,0,0.01)] transition-all">
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.away="open = false" class="bg-gradient-to-tr from-blue-500 to-blue-700 text-white rounded-2xl w-11 h-11 flex items-center justify-center font-bold text-base shadow-md shadow-blue-500/10 cursor-pointer shrink-0 focus:outline-none">
                ...
                <div x-show="open" 
                    ...
                    <div class="px-4 py-2 border-b border-slate-100">
                        ...
                        <p class="text-xs text-slate-400 font-semibold truncate mt-0.5">{{ Auth::user()->email }}</p>
                    </div>
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">
                        <i class="fa-solid fa-user-gear text-slate-400"></i> Pengaturan Akun
                    </a>
                    <form method="POST" action="{{ route('logout') }}" class="block w-full">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50 transition-colors text-left">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
            <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Skor Risiko Global (Rata-rata)</span>
            ...
            <div class="flex items-center justify-between mt-auto pt-2">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-black tracking-wide uppercase bg-slate-50 text-slate-600 border border-slate-100 group-hover:bg-blue-50 group-hover:text-blue-600 group-hover:border-blue-100 transition-colors">
        ...
        @if(Auth::user()->email === 'admin@gsc.com')
            ...
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Total Pengguna Terdaftar</span>
                <div class="text-4xl font-black text-slate-800 mt-2">{{ $totalUsers }}</div>
                <div class="flex items-center gap-1.5 text-xs font-bold mt-auto pt-2 text-indigo-600">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Total Artikel Analisis</span>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Total Dataset Pelabuhan</span>
        ...
        @else
            <!-- High-Risk Countries -->
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Negara Risiko Tinggi</span>
                <div class="text-4xl font-black text-slate-800 mt-2">{{ $highRiskCount }}</div>
                <div class="flex items-center gap-1.5 text-xs font-bold mt-auto pt-2">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Negara Risiko Menengah</span>
            ...
            <!-- Low Risk Countries -->
            <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
                <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Negara Risiko Rendah</span>
        ...
        <!-- Monitored Countries -->
        <div class="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:-translate-y-1 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300 cursor-pointer flex flex-col justify-between h-36 border border-slate-200/60 group">
            <span class="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Negara Terpantau</span>
            ...
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <div class="flex items-center gap-2.5">
                        ...
                        <span class="flex items-center gap-1.5 bg-emerald-50 text-emerald-600 text-sm font-black px-2.5 py-1 rounded-full uppercase tracking-wide">
                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block"></span>
                            Live
                        </span>
                    </div>
                    <p class="text-sm text-slate-400 font-semibold mt-0.5">Penilaian risiko visual dari simpul logistik aktif</p>
                </div>
                <div class="flex items-center gap-3">
                    <span class="flex items-center gap-1.5 text-sm font-bold text-slate-400"><span class="w-2.5 h-2.5 rounded-full bg-emerald-500"></span> Rendah</span>
                    <span class="flex items-center gap-1.5 text-sm font-bold text-slate-400"><span class="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Menengah</span>
                    <span class="flex items-center gap-1.5 text-sm font-bold text-slate-400"><span class="w-2.5 h-2.5 rounded-full bg-rose-500 animate-pulse"></span> Tinggi</span>
                </div>
            </div>
            ...
            <div class="relative">
                ...
                <div class="absolute bottom-3 left-3 z-[400] flex flex-wrap gap-2 pointer-events-none">
                    <div class="bg-slate-100/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-lg border border-white/80 flex items-center gap-1.5">
                        ...
                        <span class="text-sm font-black text-slate-700">{{ $highRiskCount }} Risiko Tinggi</span>
                    </div>
                    <div class="bg-slate-100/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-lg border border-white/80 flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-amber-500 shrink-0"></span>
                        <span class="text-sm font-black text-slate-700">{{ $mediumRiskCount }} Menengah</span>
                    </div>
                    <div class="bg-slate-100/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-lg border border-white/80 flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 shrink-0"></span>
                        <span class="text-sm font-black text-slate-700">{{ $lowRiskCount }} Rendah</span>
                    </div>
                </div>
                <!-- Floating total monitored chip -->
                <div class="absolute top-3 right-3 z-[400] pointer-events-none">
                    <div class="bg-slate-100/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-lg border border-white/80 flex items-center gap-1.5">
                        <i class="fa-solid fa-earth-asia text-sm text-blue-500"></i>
                        <span class="text-sm font-black text-slate-700">{{ $monitoredCount }} Negara</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-100 rounded-3xl border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 overflow-hidden flex flex-col">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-400 font-semibold mt-0.5">Pemantauan geografis pelabuhan logistik aktif</p>
                </div>
                <span class="flex items-center gap-1.5 bg-blue-50 text-blue-600 text-sm font-black px-2.5 py-1 rounded-full uppercase tracking-wide">
            ...
            <div class="relative">
                ...
                <div class="absolute top-2 right-2 z-[400] pointer-events-none">
                    <div class="bg-slate-100/90 backdrop-blur-sm rounded-xl px-3 py-1.5 shadow-lg border border-white/80 flex items-center gap-1.5">
                        ...
                        <span class="text-sm font-black text-slate-700">Klik pelabuhan untuk detail</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-5 bg-slate-100/70 border-t border-slate-100 flex flex-col justify-between gap-3 h-[140px]">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-2">
                    <div>
                        <span class="text-sm text-slate-400 font-bold uppercase tracking-wider">Pelabuhan Terpilih</span>
                        <h4 id="panel-port-name" class="text-base font-black text-slate-800 mt-0.5">Port of Singapore</h4>
                    </div>
                    <a href="{{ route('ports.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-black transition-colors flex items-center gap-1 shrink-0">
                        Lihat Semua Pelabuhan <i class="fa-solid fa-chevron-right text-sm"></i>
                    </a>
                </div>
                <div class="grid grid-cols-4 gap-4 border-t border-slate-200/50 pt-3">
                    <div>
                        <span class="text-sm text-slate-400 font-semibold block">Negara</span>
                        <span id="panel-port-country" class="text-base font-extrabold text-slate-700 block mt-0.5">Singapore</span>
                    </div>
                    <div>
                        <span class="text-sm text-slate-400 font-semibold block">Wilayah</span>
                        <span id="panel-port-region" class="text-base font-extrabold text-slate-700 block mt-0.5">Southeast Asia</span>
                    </div>
                    <div>
                        <span class="text-sm text-slate-400 font-semibold block">Tipe</span>
                        <span id="panel-port-type" class="text-base font-extrabold text-slate-700 block mt-0.5">Seaport</span>
                    </div>
                    <div>
                        <span class="text-sm text-slate-400 font-semibold block">Status</span>
                        <span id="panel-port-status" class="text-base font-extrabold text-emerald-600 block mt-0.5">Active</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-slate-100 rounded-3xl p-6 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col justify-between min-h-[340px]">
                    ...
                    @foreach($top5Countries->values() as $index => $tc)
                        <div class="flex items-center justify-between py-3.5">
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <span class="text-sm font-bold text-slate-700">{{ $tc->country->name }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-sm font-black {{ $tc->risk_level === 'High' ? 'text-rose-600' : ($tc->risk_level === 'Medium' ? 'text-amber-600' : 'text-emerald-600') }}">
                                    {{ $tc->total_score }}
                                </span>
                                <span class="px-2.5 py-0.5 rounded-lg text-sm font-black uppercase tracking-wide
            ...
            <div class="pt-4 border-t border-slate-100 text-center">
                <a href="{{ route('countries.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-black transition-colors flex items-center justify-center gap-1">
                    Lihat Semua Negara <i class="fa-solid fa-chevron-right text-sm"></i>
                </a>
            </div>
        </div>
        ...
        <div class="bg-slate-100 rounded-3xl p-6 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col justify-between min-h-[340px]">
            ...
            <div class="grid grid-cols-2 gap-4 flex-1">
                ...
                <div class="bg-slate-100/50 border border-slate-100 rounded-2xl p-3.5 flex flex-col justify-between">
                    <span class="text-sm text-slate-400 font-extrabold uppercase tracking-wide block">PDB Global (Total)</span>
                    <div class="text-base font-black text-slate-800 mt-2">${{ number_format($globalGdpValue / 1e12, 1) }} T</div>
                    <span class="text-sm font-bold text-emerald-500 flex items-center gap-1 mt-1">
                        <i class="fa-solid fa-caret-up"></i> +2.8%
                    </span>
                </div>
                <!-- Inflation -->
                <div class="bg-slate-100/50 border border-slate-100 rounded-2xl p-3.5 flex flex-col justify-between">
                    <span class="text-sm text-slate-400 font-extrabold uppercase tracking-wide block">Inflasi Global (Rata-rata)</span>
                    <div class="text-base font-black text-slate-800 mt-2">{{ number_format($globalInflation, 1) }}%</div>
                    <span class="text-sm font-bold text-rose-500 flex items-center gap-1 mt-1">
                        <i class="fa-solid fa-caret-down"></i> -0.6%
                    </span>
                </div>
                <!-- Currency -->
                <div class="bg-slate-100/50 border border-slate-100 rounded-2xl p-3.5 flex flex-col justify-between">
                    <span class="text-sm text-slate-400 font-extrabold uppercase tracking-wide block">Indeks Mata Uang Global</span>
                ...
                <!-- Trade -->
                <div class="bg-slate-100/50 border border-slate-100 rounded-2xl p-3.5 flex flex-col justify-between">
                    <span class="text-sm text-slate-400 font-extrabold uppercase tracking-wide block">Perdagangan Global (Total)</span>
        ...
        <div class="flex flex-col gap-8">
            ...
            <div class="bg-slate-100 rounded-3xl p-6 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm text-slate-400 font-extrabold uppercase tracking-wider block">Mata Uang (USD/IDR)</span>
                    <span class="text-sm text-emerald-500 font-extrabold flex items-center gap-1">
                        <i class="fa-solid fa-caret-up"></i> +0.40%
                    </span>
                </div>
                <div class="text-2xl font-black text-slate-800">16,415.00 <span class="text-sm text-slate-400 font-bold ml-0.5">IDR</span></div>
            ...
            <div class="bg-slate-100 rounded-3xl p-6 border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 flex flex-col justify-between">
                <div>
                    <h3 class="text-base font-extrabold text-slate-800 mb-2">Tinjauan Cepat Negara</h3>
                    
                    <!-- Country Dropdown Form -->
                    <form action="{{ route('dashboard') }}" method="GET" id="quickview-form" class="mb-2">
                        <div class="relative">
                            <select name="country" onchange="this.form.submit()" class="w-full bg-slate-100 border border-slate-100 text-slate-700 text-sm font-bold rounded-xl p-2.5 pr-8 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                    ...
                    @if($selectedCountry)
                        <div class="space-y-1.5 mt-3">
                            ...
                            <div class="flex items-center justify-between text-sm font-semibold py-0.5">
                                ...
                                <span class="px-2.5 py-1 rounded text-xs font-black uppercase tracking-wide
        ...
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center">
                    <i class="fa-solid fa-newspaper text-blue-500 text-base"></i>
                </div>
                <div>
                    <h3 class="text-base font-extrabold text-slate-800">Berita Terbaru</h3>
                    <p class="text-sm text-slate-400 font-semibold">Intelijen berita rantai pasok global terkini</p>
                </div>
                <span class="flex items-center gap-1.5 bg-emerald-50 text-emerald-600 text-sm font-black px-2.5 py-1 rounded-full uppercase tracking-wide">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse inline-block"></span>
                    Live
            ...
            <a href="{{ route('news.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-black transition-colors flex items-center gap-1">
                Lihat Semua Berita <i class="fa-solid fa-chevron-right text-sm"></i>
            </a>
        </div>
            ...
            @foreach($recentNews->take(3) as $news)
                <div class="flex flex-col gap-2.5 p-4 rounded-2xl bg-white border border-slate-100 hover:-translate-y-0.5 hover:shadow-md transition-all duration-200">
                    <div class="flex items-center gap-2 flex-wrap">
                        <span class="px-2.5 py-0.5 rounded-lg bg-slate-50 text-slate-600 text-sm font-bold uppercase border border-slate-100">
                            {{ $news->country->name }}
                        </span>
                        <span class="px-2.5 py-0.5 rounded-lg text-sm font-bold uppercase
                        ...
                        <span class="text-sm text-slate-400 font-bold ml-auto">
                            {{ \Carbon\Carbon::parse($news->published_at)->diffForHumans() }}
                        </span>
                    </div>
                    @if($news->url && $news->url !== '#')
                        <a href="{{ $news->url }}" target="_blank" class="group block flex-1">
                            <p class="text-base font-semibold text-slate-700 group-hover:text-blue-600 transition-colors leading-relaxed line-clamp-3">
                                {{ $news->title }}
                            </p>
                        </a>
                    @else
                        <p class="text-base font-semibold text-slate-700 leading-relaxed line-clamp-3 flex-1">
                            {{ $news->title }}
                        </p>
                    @endif
                </div>
        ...
        @if($extremeWeather)
            <div class="bg-rose-50 border border-rose-100 rounded-3xl p-6 shadow-[0_8px_30px_rgba(244,63,94,0.02)] flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div class="flex items-start gap-4 flex-1">
                    ...
                    <div>
                        <h4 class="text-base font-black text-rose-900">Peringatan Cuaca Ekstrem</h4>
                        <p class="text-sm text-rose-700 font-semibold mt-1.5 leading-relaxed">
                ...
                <a href="{{ route('weather.index') }}" class="bg-white hover:bg-rose-100 text-rose-600 font-black text-sm px-5 py-2.5 rounded-xl border border-rose-200 transition-all shrink-0">
                    Lihat Detail Cuaca
                </a>
            </div>
        @else
            <div class="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6">
                <div class="flex items-start gap-4 flex-1">
                    ...
                    <div>
                        <h4 class="text-base font-black text-emerald-900">Prakiraan Cuaca Normal</h4>
                        <p class="text-sm text-emerald-700 font-semibold mt-1.5 leading-relaxed">
                            Tidak ada kondisi cuaca ekstrem yang terdeteksi di simpul logistik. Rute pelabuhan beroperasi normal.
                        </p>
                    </div>
                </div>
                <a href="{{ route('weather.index') }}" class="bg-white hover:bg-emerald-100 text-emerald-600 font-black text-sm px-5 py-2.5 rounded-xl border border-emerald-200 transition-all shrink-0">
                    Lihat Peta Cuaca
                </a>
            </div>
        @endif
